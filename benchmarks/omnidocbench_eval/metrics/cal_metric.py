    ...
    def evaluate(self, group_info=[], save_name="default"):
        ...
        for idx, sample in enumerate(samples):
            ...
            try:
                ...
                print(
                    f"TEDS_structure_only score error for table {sample['gt_idx']}"
                    f" in {sample['img_id']}. The score is set to 0."
                )
            ...
            key = sample["img_id"] + "_" + str(sample.get("gt_idx", idx))
            per_table_score[key] = {"TEDS": score, "TEDS_structure_only": score_structure_only}
            for group in group_info:
                select_flag = True
                for k, v in group.items():
                    # gt_attribute is a list containing all merged gt attributes
    ...
    def evaluate(self, group_info=[], save_name="default"):
        ...
        for sample in samples:
            ...
            if img_id.endswith(".jpg") or img_id.endswith(".png"):
                ...
                img_name = "_".join(img_id.split("_")[:-1])
        ...
        return samples, {
            "Edit_dist": {
                "ALL_page_avg": up_total_avg.mean(),
                "edit_whole": edit_whole,
                "edit_sample_avg": edit_sample_avg,
            }
        }
