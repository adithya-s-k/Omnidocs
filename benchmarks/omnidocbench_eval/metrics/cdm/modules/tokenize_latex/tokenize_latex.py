...
def tokenize_latex(latex_code, latex_type="", middle_file=""):
    ...
    if latex_type == "formula":
        ...
        js_path = os.path.join(os.path.dirname(__file__), "preprocess_formula.js")
        cmd = r"cat %s | node %s %s > %s " % (temp_file, js_path, "normalize", middle_file)
        ...
        operators = r"\s?".join("|".join(_ops_list))
        ...
        if ret != 0:
            return False, latex_code
        os.remove(middle_file)
        js_path = os.path.join(os.path.dirname(__file__), "preprocess_tabular.js")
        cmd = r"cat %s | node %s %s > %s " % (temp_file, js_path, "tokenize", middle_file)
        ret = subprocess.call(cmd, shell=True)
        os.remove(temp_file)
        if ret != 0:
            return False, latex_code
