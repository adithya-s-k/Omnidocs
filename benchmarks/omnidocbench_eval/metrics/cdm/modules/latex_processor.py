...
def normalize_latex(latex_str, rm_trail=False):
    if "tabular" in latex_str:
        latex_type = "tabular"
    else:
        latex_type = "formula"

    if rm_trail:
        latex_str = remove_trailing_latex(latex_str)
    latex_str = latex_str.strip().replace(r"\pmatrix", r"\mypmatrix").replace(r"\matrix", r"\mymatrix")

    # TODO \raggedright \arraybackslash, these align method, difficult to handle, remove it.
    for item in ["\\raggedright", "\\arraybackslash"]:
        latex_str = latex_str.replace(item, "")
    ...
    old_token = re.findall(pattern, latex_str, re.DOTALL)
    ...
    for bef, aft in zip(old_token, new_token):
        latex_str = latex_str.replace(bef, aft)
    ...
    if latex_type == "tabular":
        ...
        latex_str = latex_str.replace("\\begin {array}", "\\begin{array}")
        latex_str = latex_str.replace("\\end {array}", "\\end{array}")
        l_split = latex_str.split(" ")
        idx = 0
        while idx < len(l_split):
            token = l_split[idx]
            if token == "\\begin{tabular}":
                ...
                break
            idx += 1
        latex_str = " ".join(l_split)

        # TODO some complex format, hart to deal with re.match, so
        # using brace match, such as：\cmidrule ( l { 3 p t } r { 3 p t } ) { 1 - 1 }
        l_split = latex_str.split(" ")
        idx = 0
        while idx < len(l_split):
            token = l_split[idx]
    ...
    old_token = re.findall(pattern, latex_str, re.DOTALL)
    new_token = [
        item.replace("\\begin{array} ", "<s>").replace(" ", "").replace("<s>", "\\begin{array} ") for item in old_token
    ]
    for bef, aft in zip(old_token, new_token):
        latex_str = latex_str.replace(bef, aft)
    ...
    latex_str = " " + latex_str + " "
    latex_str = latex_str.replace(" \\ldots ", " . . . ")
    latex_str = latex_str.replace(" \\cdots ", " . . . ")
    latex_str = latex_str.replace(" \\dots ", " . . . ")
    latex_str = latex_str.replace(" \\dotsb ", " . . . ")
    latex_str = latex_str.replace(" \\log ", " \\mathrm { l o g } ")
    latex_str = latex_str.replace(" \\exp ", " \\mathrm { e x p } ")
    latex_str = latex_str.replace(" \\sin ", " \\mathrm { s i n } ")
    latex_str = latex_str.replace(" \\cos ", " \\mathrm { c o s } ")
    latex_str = latex_str.replace(" \\tan ", " \\mathrm { t a n } ")
    latex_str = latex_str.replace(" \\tanh ", " \\mathrm { t a n h } ")
    latex_str = latex_str.replace(" \\cosh ", " \\mathrm { c o s h } ")
    latex_str = latex_str.replace(" \\sinh ", " \\mathrm { s i n h } ")
    ...
    latex_str = latex_str.replace("\\lefteqn", "")

    # TODO \footnote can not seem as ONE_Tail_Invisb_Tokens(usually this type
    # token add color by \mathrm {\color(x)}, yet \footnode should be \color{\footnote{x}}),
    # so we simple change it to "^".
    latex_str = latex_str.replace("\\footnote ", "^ ")

    # TODO \' can not be rendered separately(cause to different visulize performence),
    # so we take these tokens as one token such as \' e -> \'e, on the other hand,
    # if { after \' then render them separately.
    pattern = r"\\\' [^{] "
    old_token = re.findall(pattern, latex_str, re.DOTALL)
    new_token = [item.replace(" ", "") for item in old_token]
    for bef, aft in zip(old_token, new_token):
    ...
    new_token = [item.replace(" ", "") for item in old_token]
    for bef, aft in zip(old_token, new_token):
        latex_str = latex_str.replace(bef, aft[0:-1] + " {")
    ...
    new_token = [item.replace(" ", "") for item in old_token]
    for bef, aft in zip(old_token, new_token):
        latex_str = latex_str.replace(bef, "{ " + aft[1:-1] + " }")
    ...
    # ** for easier add color, the original color should be removed,
    # there are two type of color for now: \color[rgb]{0, 1, 0} and \color{red}
    _color_pat = r"[ \[\]RGBrgb]+{ [A-Za-z 0-9,!]+ } "
    pattern = (
        r"\\colorbox"
        + _color_pat
        + r"|\\color"
    ...
    for bef in old_token:
        latex_str = latex_str.replace(bef, "")
    ...
    while idx < len(l_split):
        ...
        if token in ONE_Tail_Tokens + ONE_Tail_Invisb_Tokens:
            ...
            # ** normalize special tokens such as \sqrt, fill the missing [] {} in \sqrt [] {},
            # yet the [] is optional, for example: \sqrt A B -> \sqrt {A} B and \sqrt [A] B -> \sqrt [A] {B}
            ...
            # ** normalize special tokens such as \frac, add missing brace in \frac {A} {B} for example:
...
def token_add_color_rgb(l_split, idx, token_list, brace_color=False):
    ...
    if not token:
        ...
        l_split = (
            l_split[:idx] + [color_token + token] + l_split[idx + 1 : den_end + 1] + ["}"] + l_split[den_end + 1 :]
        )
        ...
        if token != "\\underbrace" and num_end + 1 < len(l_split) and l_split[num_end + 1] == "_":
            ...
            l_split = (
                l_split[:idx] + [color_token + token] + l_split[idx + 1 : num_end + 1] + ["}"] + l_split[num_end + 1 :]
            )
        token_list.append(token)
        next_idx = idx + 1
    elif token in ONE_Tail_Invisb_Tokens:
