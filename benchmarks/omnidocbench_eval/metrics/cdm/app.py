...
def evaluation(user_id="test"):
    ...
    for basename in gt_basename_list:
        ...
        if src.shape[0] <= min_samples:
            ...
            for i in range(max_iter):
                if src[~inliers].shape[0] <= min_samples:
                    break
    ...
    metrics_res = {"mean_score": round(np.mean(acc_list), 3), "details": metrics_per_img}
...
def gradio_reset_single():
    return (
        gr.update(value=None, placeholder="type gt latex code here"),
        gr.update(value=None, placeholder="type pred latex code here"),
...
def select_example1():
    ...
    pred = "y = 2z + 3x"
    return (
        gr.update(value=gt, placeholder="type gt latex code here"),
        gr.update(value=pred, placeholder="type pred latex code here"),
    )


def select_example2():
    gt = (
        "r = \\frac { \\alpha } { \\beta } \\vert \\sin \\beta"
        " \\left( \\sigma _ { 1 } \\pm \\sigma _ { 2 } \\right) \\vert"
    )
...
def select_example3():
    gt = (
        "\\begin{array} { r l r } & { } & { \\mathbf { J } _ { L } = "
        "\\left( \\begin{array} { c c } { 0 } & { 0 } \\\\ { v _ { n } } & { 0 } "
        "\\end{array} \\right) , ~ \\mathbf { J } _ { R } = "
        "\\left( \\begin{array} { c c } { u _ { n - 1 } } & { 0 } \\\\ "
        "{ 0 } & { 0 } \\end{array} \\right) , ~ } \\\\ & { } & "
        "{\\mathbf { K } = \\left( \\begin{array} { c c } { V _ { n - 1 } } & "
        "{ u _ { n } } \\\\ { v _ { n - 1 } } & { V _ { n } } "
        "\\end{array} \\right) , } \\end{array}"
    )
    pred = (
        "\\mathbf{J}_{U}={\\left(\\begin{array}{l l}{0}&{0}\\\\ {v_{n}}&{0}"
        "\\end{array}\\right)}\\,,\\ \\mathbf{J}_{R}={\\left(\\begin{array}{l l}"
        "{u_{n-1}}&{0}\\\\ {0}&{0}\\end{array}\\right)}\\,,\\mathbf{K}={"
        "\\left(\\begin{array}{l l}{V_{n-1}}&{u_{n}}\\\\ {v_{n-1}}&{V_{n}}"
        "\\end{array}\\right)}\\,,"
    )
    return (
        gr.update(value=gt, placeholder="type gt latex code here"),
...
if __name__ == "__main__":
    ...
    with gr.Blocks() as demo:
        ...
        clear_single.click(
            gradio_reset_single,
            inputs=None,
            outputs=[gt_input, pred_input, score_output, recall_output, recision_output, vis_output],
        )
        submit_single.click(
            calculate_metric_single,
            inputs=[gt_input, pred_input],
            outputs=[score_output, recall_output, recision_output, vis_output],
        )

        gr.Button(
            value=(
                "Batch Run: upload a json file and batch processing, "
                "this may take some times according to your latex amount and length."
            ),
            interactive=False,
            variant="primary",
        )
