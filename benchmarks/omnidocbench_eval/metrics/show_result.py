...
def get_full_labels_results(samples):
    ...
    for sample in samples:
        ...
        # If there are merged cases, calculate based on the set of all labels involved after merging
...
#     for sample in samples:
#         img_name = (sample['img_id'] if sample['img_id'].endswith('.jpg')
#                     else '_'.join(sample['img_id'].split('_')[:-1]))
...
def get_page_split(samples, page_info):  # Page level metric
    ...
    for sample in samples:
        # img_name = sample['img_id'][:-4] if sample['img_id'].endswith('.jpg')
        # or sample['img_id'].endswith('.png') else '_'.join(sample['img_id'].split('_')[:-1])
        img_id = sample["img_id"]
        img_name = img_id[:-4] if img_id.lower().endswith((".jpg", ".jpeg", ".png", ".pdf")) else img_id
        page_info_s = page_info[img_name]
        if not sample.get("metric"):
            continue
        for metric, score in sample["metric"].items():
    ...
    if result_list.get("Edit_dist"):  # 只有Edit_dist需要进行page level的计算
        ...
        # up_total_avg = df.groupby(["attribute"]).apply(
    ...
    for metric in result_list.keys():
        ...
        # 页面内部平均以后，再页面间的平均
        page_avg = (
            df.groupby(["image_name", "attribute"]).apply(lambda x: x["score"].mean()).groupby("attribute").mean()
        )
