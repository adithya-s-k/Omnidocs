...
class Md2MdDataset:
    ...
    def get_order_paired(self, order_match_s, img_name):
        ...
        # During truncation merging, some discarded classes may be merged in; remove them when calculating edit distance
        ...
        for sample_name in tqdm(os.listdir(gt_folder)):
            ...
            if gt_dataset.get("equation_isolated"):
                display_formula_match_s = match_gt2pred(
                    gt_dataset["equation_isolated"],
                    pred_dataset["equation_isolated"],
                    "formula",
                    img_name,
                )
                # Remove extra preds; inline formulas included for matching but excluded from results
                display_formula_match_s = [
                    x
                    for x in display_formula_match_s
                    if x["gt_idx"] != [""] and x["gt_category_type"] != "equation_inline"
                ]
                display_formula_match.extend(display_formula_match_s)
            # By default the model won't output both latex and html; table format in GT must match Pred
            if gt_dataset.get("latex_table") and pred_dataset.get("latex_table"):
