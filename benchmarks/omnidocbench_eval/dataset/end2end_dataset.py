...
class End2EndDataset:
    ...
    def get_page_elements(self, selected_annos):
        ...
        relations = (selected_annos.get("extra") or {}).get("relation") or []
        for relation in relations:  # Handle truncated text issues
    ...
    # 对gt和预测结果进行匹配，调用 process_get_matched_elements 函数进行匹配处理，
    # 最终将匹配结果整理成一个字典返回
    def get_matched_elements(self, gt_samples, pred_folder):
        ...
        for sample in process_bar:
            ...
            # 对单个样本匹配，根据不同的元素类型（如文本块、显示公式、表格等），
            # 使用指定的匹配方法将gt与预测结果进行匹配，并返回匹配结果
        ...
        for item in display_formula_match:
            ...
            if pred_category_type not in ["equation_inline", "equation_isolated", ""]:
                ...
                # item['norm_gt'] = LatexNodes2Text().latex_to_text(norm_gt)  # 错了，
                # 这里的norm gt是跑的normalized_formula函数，所以再跑latex2unicode会报错
    ...
    def process_get_matched_elements(self, sample, pred_content, img_name, save_time):
        ...
        gt_mix = self.get_page_elements_list(
            gt_page_elements,
            [
                "text_block",
                "title",
        ...
        if gt_page_elements.get("table"):
            ...
            if latex_table_len == html_table_len and latex_table_len == 0:
                ...
                html_table_match_s, unmatch_table_pred = match_gt2pred_simple(gt_table, [], "html_table", img_name)
