...
from utils.data_preprocess import clean_string, normalized_formula, normalized_table, textblock2unicode
...
class RecognitionTextDataset:
    ...
    def load_data(self, gt_file, pred_folder):
        ...
        for gt in gts:
            ...
            samples.append({"gt": gt_text, "pred": pred_text, "img_id": img_name})
        return samples
...
class RecognitionFormulaDataset:
    ...
    def load_data(self, gt_file, pred_file):
        ...
        for gt, pred in zip(norm_gts, norm_preds):
            samples.append({"gt": gt, "pred": pred, "img_id": img_id})
            img_id += 1
