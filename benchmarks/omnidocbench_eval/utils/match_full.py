...
class FuzzyMatch:
        ...
        for free_gt_idx in gs_free_arr:
            for pred_idx in range(len(self._preds)):
                if (pred_idx, free_gt_idx) in match_gs_free_pred_combine_ret_h:
                    edit_dis_q.append(
                        (
                            match_gs_free_pred_combine_ret_h[(pred_idx, free_gt_idx)],
                            MatchPair(pred_idx, free_gt_idx),
                        )
                    )
    ...
    def _combine_match(self, window_arr, line_arr, window_used_s, line_used_s):
        ...
        for i in range(len(window_arr)):
            ...
            if edit_dis < len(window_arr[i]) * match_edit_dis_ratio or (
                abs_diff_len >= len(window_arr[i]) and abs_diff_char_count >= edit_dis
            ):
        ...
        for i in range(len(window_arr)):
            ...
            beyond_sigma = sorted(
                [
                    (j, edit_dis, pos)
                    for j, edit_dis, pos in edit_dis_pair
                    if mean - sigma_multiple * std_var >= edit_dis
                ],
                key=lambda x: x[1],
            )
...
def match_gt2pred_full(gts, predications):
    ...
    for gt_idx in gt_one_pred.keys():
        ...
        ret.append({"gt_idx": [gt_idx], "gt": gts[gt_idx], "pred_idx": [pred_idx], "pred": predications[pred_idx]})
    seen_gt_s.add(gt_idx)
