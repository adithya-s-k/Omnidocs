...
def split_gt_equation_arrays(data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in data:
        if item.get("category_type") == "equation_isolated":
            # Resolve the actual latex content — some GT annotations store it
            # in 'latex', others (multilingual data) store it in 'text' with
            # $$ delimiters. Normalise here so the rest of the logic is uniform.
            latex_content = item.get("latex") or item.get("text", "") or ""

            if "\\begin{array" in latex_content:
                match = ARRAY_RE.search(latex_content)
                if match:
                    spec = match.group("spec")
                    if not is_all_l(spec):
                        output.append(item)
                    ...
                    for idx, line in enumerate(lines, start=1):
                        new_item = deepcopy(item)
                        # Write back to 'latex' so downstream (get_gt_pred_lines)
                        # picks it up correctly via item.get("latex")
                        new_item["latex"] = f"\\[{line}\\]"
                        new_item["order"] = round(base_order + idx / 10, 1)
                        output.append(new_item)
                    continue
...
def match_gt2pred_quick(gt_items, pred_items, line_type, img_name):
    ...
    # gt_lines, norm_gt_lines, gt_cat_list, pred_lines,
    # norm_pred_lines= get_gt_pred_lines(gt_items, pred_items, line_type)
    (gt_lines, norm_gt_lines, gt_cat_list, pred_lines, norm_pred_lines, gt_items, pred_items) = get_gt_pred_lines(
        gt_items, pred_items, None
    )
            ...
            match_list.append(
                {
                    ...
                    "pred_idx": [""],
                    "pred": "",
                    "gt_position": [gt_items[gt_idx].get("order") or gt_items[gt_idx].get("position", [""])[0]],
                    "pred_position": "",
        ...
        return [
            {
                ...
                "pred_idx": [0],
                "pred": pred_lines[0],
                "gt_position": [gt_items[0].get("order") or gt_items[0].get("position", [""])[0]],
    ...
    if len(ignore_gt_lines) > 0:
        ...
        for entry in merged_ignore_results:
            ...
            entry["pred_position"] = (
                ignores_pred_items[entry["pred_idx"][0]]["position"][0] if entry["pred_idx"] != [""] else ""
            )
            entry["gt"] = "".join([ignores_ori_gt_lines[_] for _ in entry["gt_idx"]]) if entry["gt_idx"] != [""] else ""
            entry["pred"] = (
                "".join([ignores_ori_pred_lines[_] for _ in entry["pred_idx"]]) if entry["pred_idx"] != [""] else ""
            )
            entry["norm_gt"] = "".join([ignore_gt_lines[_] for _ in entry["gt_idx"]]) if entry["gt_idx"] != [""] else ""
            entry["norm_pred"] = (
                "".join([ignore_pred_lines[_] for _ in entry["pred_idx"]]) if entry["pred_idx"] != [""] else ""
            )
            if entry["gt_idx"] != [""]:
                ignore_type = [
                    "figure_caption",
                ...
                gt_cagegory_clean = [
                    ignores_gt_cat_list[_] for _ in entry["gt_idx"] if ignores_gt_cat_list[_] not in ignore_type
                ]
                if gt_cagegory_clean:
                    entry["gt_category_type"] = Counter(gt_cagegory_clean).most_common(1)[0][0]
                else:
                    entry["gt_category_type"] = Counter([ignores_gt_cat_list[_] for _ in entry["gt_idx"]]).most_common(
                        1
                    )[0][0]
            else:
            ...
            entry["pred_category_type"] = (
                get_pred_category_type(entry["pred_idx"][0], ignores_pred_items) if entry["pred_idx"] != [""] else ""
            )
            entry["gt_attribute"] = (
                [ignores_gt_items[_].get("attribute", {}) for _ in entry["gt_idx"]] if entry["gt_idx"] != [""] else [{}]
            )
    ...
    matches, unmatched_gt_indices, unmatched_pred_indices = process_matches(
        matched_col_idx, row_ind, cost_list, no_ignores_gt_lines, no_ignores_pred_lines, no_ignores_ori_pred_lines
    )
    # print("-------------matches-------------")
    # print(matches)
    ...
    for entry in merged_results:
        if entry["gt_idx"] != [""]:
            ...
            gt_cagegory_clean = [
                no_ignores_gt_cat_list[_] for _ in entry["gt_idx"] if no_ignores_gt_cat_list[_] not in ignore_type
            ]
            if gt_cagegory_clean:
                entry["gt_category_type"] = Counter(gt_cagegory_clean).most_common(1)[0][0]
            else:
                entry["gt_category_type"] = Counter([no_ignores_gt_cat_list[_] for _ in entry["gt_idx"]]).most_common(
                    1
                )[0][0]
        else:
            entry["gt_category_type"] = ""
            entry["pred_category_type"] = (
                get_pred_category_type(entry["pred_idx"][0], no_ignores_pred_items) if entry["pred_idx"] != [""] else ""
            )
            if entry["pred_category_type"] == "equation_inline":
        ...
        entry["pred_position"] = (
            no_ignores_pred_items[entry["pred_idx"][0]]["position"][0] if entry["pred_idx"] != [""] else ""
        )
        #  0507 多行公式拼接修改
        if entry["gt_category_type"] == "equation_isolated" and len(entry["gt_idx"]) > 1:
            ...
            mutli_formula = "\\begin{array}{l} " + mutli_formula + r" \end{array}"
            entry["gt"] = mutli_formula
        else:
            entry["gt"] = (
                "".join([no_ignores_ori_gt_lines[_] for _ in entry["gt_idx"]]) if entry["gt_idx"] != [""] else ""
            )
        entry["pred_category_type"] = (
            get_pred_category_type(entry["pred_idx"][0], no_ignores_pred_items) if entry["pred_idx"] != [""] else ""
        )
        entry["gt_attribute"] = (
            [no_ignores_gt_items[_].get("attribute", {}) for _ in entry["gt_idx"]] if entry["gt_idx"] != [""] else [{}]
        )
        entry["img_id"] = img_name
        ...
        if "equation" in entry["pred_category_type"] and len(entry["pred_idx"]) > 1:
            ...
            entry["pred"] = (
                "".join([no_ignores_ori_pred_lines[_] for _ in entry["pred_idx"]]) if entry["pred_idx"] != [""] else ""
            )

        entry["norm_gt"] = "".join([no_ignores_gt_lines[_] for _ in entry["gt_idx"]]) if entry["gt_idx"] != [""] else ""
        entry["norm_pred"] = (
            "".join([no_ignores_pred_lines[_] for _ in entry["pred_idx"]]) if entry["pred_idx"] != [""] else ""
        )
    ...
    # merged_results = merge_duplicates_add_unmatched(converted_results, norm_gt_lines,
    # norm_pred_lines, gt_lines, pred_lines, all_gt_indices, all_pred_indices)

    # for entry in merged_results:
    #         entry['gt_idx'] = [entry['gt_idx']] if not isinstance(entry['gt_idx'], list) else entry['gt_idx']
    #         entry['pred_idx'] = [entry['pred_idx']] if not isinstance(entry['pred_idx'], list) else entry['pred_idx']
    #         entry['gt_position'] = [gt_items[_].get('order') if gt_items[_].get('order')
    #  else gt_items[_].get('position', [""])[0] for _ in entry['gt_idx']] if entry['gt_idx']
    # != [""] else [""]
    #         entry['pred_position'] = pred_items[entry['pred_idx'][0]]['position'][0]
    ...
    #         if entry['gt_idx'] != [""]:
    #             ignore_type = ['figure_caption', 'figure_footnote', 'table_caption',
    #  'table_footnote', 'code_algorithm', 'code_algorithm_caption', 'header', 'footer',
    ...
    #                 entry['gt_category_type'] = Counter(gt_cagegory_clean).most_common(1)[0][0]
    #             else:
    #                 entry['gt_category_type'] = Counter([gt_cat_list[_]
    # for _ in entry['gt_idx']]).most_common(1)[0][0]
    #         else:
    #             entry['gt_category_type'] = ""
    #         entry['pred_category_type'] = get_pred_category_type(entry['pred_idx'][0],
    #  pred_items) if entry['pred_idx'] != [""] else ""
    #         entry['gt_attribute'] = [gt_items[_].get("attribute", {}) for _ in entry['gt_idx']]
...
def formula_format(formula_matches, img_name):
    return [
        {"gt": item["gt"], "pred": item["pred"], "img_id": f"{img_name}_{i}"} for i, item in enumerate(formula_matches)
    ]
    ...
    for gt_idx in unmasked_gt_idx:
        ...
        for pred_idx in unmasked_pred_idx:
            ...
            dist = levenshtein_distance(norm_gt_lines[gt_idx], matched_line) / max(
                len(matched_line), len(norm_gt_lines[gt_idx])
            )
    ...
    if unmatched_pred_indices:
        if unmatched_gt_indices:
            distance_matrix = [
                ...
                [
                    levenshtein_distance(norm_gt_lines[gt_idx], norm_pred_lines[pred_idx])
                    / max(len(norm_gt_lines[gt_idx]), len(norm_pred_lines[pred_idx]))
                    for pred_idx in unmatched_pred_indices
                ]
