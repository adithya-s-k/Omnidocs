...
from .data_preprocess import clean_string, normalized_formula, textblock2unicode
        ...
        for i, gt_line in enumerate(gt_lines):
            for j, matched_line in enumerate(matched_lines):
                if len(gt_line) == 0 and len(matched_line) == 0:
                    ...
                    distance_matrix[i][j] = Levenshtein.distance(gt_line, matched_line) / max(
                        len(matched_line), len(gt_line)
                    )
    ...
    if line_type in ["html_table", "latex_table"]:
        for item in gt_mix:
            ...
            if item.get("content"):
                ...
                gt_lines.append(str(item.get("latex", item.get("html", ""))))
        ...
        for item in gt_mix:
            ...
            # formula
                ...
                latex_content = item.get("latex") or item.get("text", "")
                gt_lines.append(str(latex_content))
                norm_gt_lines.append(normalized_formula(str(latex_content)))
...
def match_gt2pred_simple(gt_items, pred_items, line_type, img_name):
        ...
        match_list.append(
            {
                ...
                "pred_category_type": get_pred_category_type(
                    pred_idx_list[0], pred_items
                ),  # get the first pred's category
                "gt_attribute": [{}],
                "edit": 1,
                "img_id": img_name,
            }
    ...
    pred_idx_list = [pred_idx for pred_idx in range(len(norm_pred_lines)) if pred_idx not in col_ind]
    if pred_idx_list:
        if line_type in ["html_table", "latex_table"]:
            unmatch_table_pred = []
            for i in pred_idx_list:
                ...
                text_block = [
                    re.sub(r"\$\\cdot\$", "", item.string).strip() for item in soup.findAll("td") if item.string
                ]
            ...
            match_list.append(
                {
                    ...
                    "gt_category_type": "",
                    # get the first pred's category
                    "pred_category_type": get_pred_category_type(pred_idx_list[0], pred_items),
                    "gt_attribute": [{}],
                    "edit": 1,
                    "img_id": img_name,
                }
...
def match_gt2pred_no_split(gt_items, pred_items, line_type, img_name):
    ...
    pred_line_with_position = [
        (pred_item["position"], pred_line, pred_norm_line)
