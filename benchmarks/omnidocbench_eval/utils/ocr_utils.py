...
def merge_spans_to_line(spans):
    if len(spans) == 0:
        ...
        for span in spans[1:]:
            ...
            if span["type"] in ["interline_equation"] or any(s["type"] in ["interline_equation"] for s in current_line):
                # Start a new line
...
def merge_para_with_text(para_block):
    ...
    for line in para_block["lines"]:
        ...
        for span in line["spans"]:
            ...
            if content != "":
                if "zh" in line_lang:  # For documents with one character per span, character-level
                    # language detection is inaccurate, need to detect language for whole line
...
def normalize_format(block, pred_spans):
    ...
    for span in pred_spans:
        spans.append({"type": span["category_type"], "bbox": poly2bbox(span["poly"]), "content": span["text"]})

    block_type = block["category_type"]
    block_bbox = poly2bbox(block["poly"])
    block_dict = {"type": block_type, "bbox": block_bbox, "spans": spans}
    return block_dict
