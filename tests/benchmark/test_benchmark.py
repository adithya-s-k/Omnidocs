...
def _omnidocbench_remote(extractor_factory, sample_dicts: list) -> list:
    ...
    from benchmarks.omnidocbench.runner import run_inference, write_md_files
...
def _run_olmocr_benchmark(
    ...
    for mid, future in futures.items():
        ...
        if future is None:
            results = [
                OlmResult(case_id=c.case_id, split=c.split, check_type=c.check_type,
                          model=mid, passed=False, latency_s=0.0,
...
def _multilingual_remote(extractor_factory, languages: list, max_per_language) -> dict:
    ...
    from benchmarks.multilingual.runner import run_inference, write_md_files
    from dataclasses import asdict
...
def main(
    ...
    if benchmark == "omnidocbench":
        ...
        target_langs = [l.strip() for l in languages.split(",") if l.strip()] if languages else ALL_LANGUAGES
        bad_langs    = [l for l in target_langs if l not in ALL_LANGUAGES]
        if bad_langs:
