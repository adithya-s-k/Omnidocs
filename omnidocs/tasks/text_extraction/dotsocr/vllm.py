...
class DotsOCRVLLMConfig(BaseModel):
    ...
    max_model_len: int = Field(
        default=32768, ge=1024, description="Maximum context length (Dots OCR supports up to 32K)"
    )
    ...
    enforce_eager: bool = Field(
        default=False, description="Use eager execution mode (disable CUDA graphs for faster cold start)"
    )
