...
class UniMERNetExtractor(BaseLatexExtractor):
    ...
    def _check_model_exists(self) -> bool:
        """Check if the model files exist at the specified path."""
        ...
        for file in self.REQUIRED_FILES:
            if not (self.model_path / file).exists():
                if self.show_log:
                    logger.warning(f"Required file {file} not found in {self.model_path}")
                return False
    ...
    def _download_model(self) -> Path:
        """Download UniMERNet model from HuggingFace Hub."""
        try:
            if snapshot_download is None:
                raise ImportError(
                    "huggingface_hub is not installed. Please install it with: "
            ...
            if self.show_log:
                logger.info(f"Downloading UniMERNet model from {self.MODEL_REPO}...")
            ...
            if self.show_log:
                logger.success(f"UniMERNet model downloaded to {self.model_path}")

            return Path(model_dir)

        except Exception as e:
            logger.error(f"Failed to download UniMERNet model: {str(e)}")
            raise RuntimeError(f"Failed to download UniMERNet model: {str(e)}")
